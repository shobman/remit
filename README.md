...
To pin the bootstrap to this release, place `REMIT_REF=v0.4.10` immediately before `sh`, the
last command in the pipeline, so `get-remit.sh` receives the variable:

```sh
curl -fsSL https://raw.githubusercontent.com/shobman/remit/main/get-remit.sh | REMIT_REF=v0.4.10 sh -s -- /path/to/your-repository
```
