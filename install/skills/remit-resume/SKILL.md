...
```sh
sh "$(git rev-parse --show-toplevel)/bin/remit" stop <slug>
```

`resume` is also the one verb that continues a run that CRASHED — a row the board shows as
`crashed`. Run it only on his word about that item, exactly as above; a crash is never a reason
to resume something he has not asked you to.
